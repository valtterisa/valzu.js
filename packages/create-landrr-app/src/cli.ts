...
function createProject(projectName: string, template: TemplateName): void {
  ...
  if (pkg.includes("workspace:*")) {
    pkg = pkg
      ...
      .replace(/"@landrr\/blocks"\s*:\s*"workspace:\*"/g, '"@landrr/blocks": "^5.0.0"')
      .replace(/"valzu-core"\s*:\s*"workspace:\*"/g, '"@landrr/core": "^6.0.0"')
      .replace(/"valzu-blocks"\s*:\s*"workspace:\*"/g, '"@landrr/blocks": "^5.0.0"')
...
async function main(): Promise<void> {
  ...
  if (!projectName) {
    console.error("Usage: create-landrr-app <project-name> [--template blank|landing|minimal|product]");
    process.exit(1);
  }
