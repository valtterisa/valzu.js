...
function createProject(projectName: string, template: TemplateName): void {
  ...
  if (pkg.includes("workspace:*")) {
    pkg = pkg
      .replace(/"@landr\/core"\s*:\s*"workspace:\*"/g, '"@landr/core": "^3.0.0"')
      .replace(/"@landr\/blocks"\s*:\s*"workspace:\*"/g, '"@landr/blocks": "^2.0.0"')
      .replace(/"valzu-core"\s*:\s*"workspace:\*"/g, '"@landr/core": "^3.0.0"')
      .replace(/"valzu-blocks"\s*:\s*"workspace:\*"/g, '"@landr/blocks": "^2.0.0"')
...
async function main(): Promise<void> {
  ...
  if (!projectName) {
    console.error("Usage: create-landr-app <project-name> [--template blank|landing|minimal|product]");
    process.exit(1);
  }
